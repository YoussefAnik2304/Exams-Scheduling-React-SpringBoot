...
import EventSponsorView from "@/components/EventSponsorView.tsx";
import {Event} from "@/types/event.ts";

type EventSponsorView = {
    event: Event
}
export default function EventSponsorViewPage({event}: EventSponsorView) {
                ...
                {event.sponsors && event.sponsors.length > 0 ?
                    (<div className="grid md:grid-cols-3 gap-3">
                        {event.sponsors?.map(item => (
                            <EventSponsorView key={item.sponsorId} sponsor={item}/>
                        ))}
                    </div>)
                    :
                    (<div
