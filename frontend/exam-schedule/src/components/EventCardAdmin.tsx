...
export default function ProfCardAdmin({prof} : ProfCardProp) {
    const {publishProf} = useProfs();
        ...
        <Card className="flex flex-col justify-between gap-y-5 bg-card rounded-xl p-3 shadow-sm border-0">
            <div className="space-y-4">
                <CardHeader className="h-48 w-full rounded-lg overflow-hidden p-0">
                    <img className="h-full w-full object-cover" src={HOST + prof.thumbnail}/>
                </CardHeader>
                <CardContent className="p-0">
                    <p className="text-lg font-medium mb-4">{prof.profName}<Badge
                        className="ml-2 bg-green-400 hover:bg-green-400">{prof.paying ? "Paying" : "Free"}</Badge></p>
                    <div className="flex flex-col gap-y-2 font-medium">
                        <div className="flex items-center gap-x-2">
                            ...
                            <span className="text-sm">{format(new Date(prof.startDate), "yyyy-MM-dd")}</span>
                        </div>
                        <div className="flex items-center gap-x-2">
                            ...
                            <span className="text-sm">{prof.profLocation}</span>
                        </div>
                        {prof.paying &&
                            <div className="flex items-center gap-x-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                                ...
                                <span className="text-sm">{prof.subscriptionPrice} DH</span>
                            </div>
                        }
                    </div>
                </CardContent>
            ...
            <CardFooter className="grid grid-cols-2 gap-x-2 p-0">
                {published ? (
                    <Button onClick={() => handlePublish(false)} className="space-x-1.5 rounded-lg w-full">
                    ...
                    <Button onClick={() => handlePublish(true)} className="space-x-1.5 rounded-lg w-full">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                             stroke="currentColor" className="size-5">
                            <path strokeLinecap="round" strokeLinejoin="round"
                ...
                <Link to="/admin/prof/edit" state={routeState}>
