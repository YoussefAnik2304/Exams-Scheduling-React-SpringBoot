...
interface ProfCardProp {
    prof: Prof,
    forUser: boolean,
}
export default function ProfCard({prof, forUser} : ProfCardProp) {
    const routeState = {
        profId: prof.profId
    }
    console.log(prof);
        ...
        <Card className="flex flex-col justify-between gap-y-5 bg-card rounded-xl p-3 shadow-sm border-0">
            <div className="space-y-4">
                <CardHeader className="h-48 w-full rounded-lg overflow-hidden p-0">
                    <img className="h-full w-full object-cover" src={HOST + prof.profImage}/>
                </CardHeader>
                <CardContent className="p-0">
                    <p className="text-lg font-medium mb-4">{prof.profLastName}
                        <Badge className="ml-2 bg-green-400 hover:bg-green-400">{prof.archived ? "Archived" : "UnArchived"}</Badge>
                    </p>
                    <div className="flex flex-col gap-y-2 font-medium">
                        ...
                        <div className="flex items-center gap-x-2">
                            ...
                            <div className="flex items-center gap-x-2">
                                ...
                                <span className="text-sm">{prof.profGroup} </span>
                            </div>
                    </div>
                </CardContent>
            </div>
            <CardFooter className="grid grid-cols-2 gap-x-2 p-0">
                {forUser &&
                    <AlertDialog>
                        <AlertDialogTrigger className="w-full">
                            <Button className="space-x-1.5 rounded-lg bg-red-500 hover:bg-red-600 w-full">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     strokeWidth={1.5}
                                     stroke="currentColor" className="size-5">
                                    <path strokeLinecap="round" strokeLinejoin="round"
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Unsubscribe Confirmation</AlertDialogTitle>
                                <AlertDialogDescription>
                                    This is a final decision. Confirming this action will permanently remove the
                                    prof from your subscriptions list and your payment cannot be refunded if
                                    the prof subscription is payed.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                ...
                <Link to="/user/prof/view" state={routeState}>
