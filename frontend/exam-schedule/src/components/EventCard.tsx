...
export default function EventCard({event, forUser, subscriptionId} : EventCardProp) {
    const {createFreeSubscription, removeSubscription} = useSubscription();
            ...
            <CardFooter className="grid grid-cols-2 gap-x-2 p-0">
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    ...
                                    event from your subscriptions list and your payment cannot be refunded if
                                    the event subscription is payed.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter className="space-x-2">
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction onClick={handleUnsubscribe}
                                                   className="bg-destructive hover:bg-red-600 ml-0 md:ml-2">Continue</AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
                }
                {!forUser && (
                    event.subscribedUsers?.includes(userId) ? (
                        <Button className="space-x-1.5 rounded-lg w-full bg-amber-500 hover:bg-amber-500">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                 strokeWidth={1.5}
                                 stroke="currentColor" className="size-5">
                                <path strokeLinecap="round" strokeLinejoin="round"
